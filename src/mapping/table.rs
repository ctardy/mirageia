...
impl MappingTable {
    ...
    /// Cherche si un pseudonyme existe déjà pour cette valeur originale.
    ...
    /// Insère un nouveau mapping original → pseudonyme.
    ...
    /// Cherche la valeur originale pour un pseudonyme donné.
    ...
    /// Retourne tous les pseudonymes connus avec leur valeur originale déchiffrée.
    /// Triés par longueur de pseudonyme décroissante (pour le remplacement prioritaire).
    pub fn all_pseudonyms_sorted(&self) -> Vec<(String, String)> {
        ...
        // Trier par longueur décroissante du pseudonyme
    ...
    /// Hash SHA-256 d'une valeur originale (jamais stockée en clair).
...
mod tests {
    ...
    fn test_duplicate_original_returns_same_pseudonym() {
        ...
        // Vérifier que lookup retourne le pseudonyme existant
        let pseudo = table.lookup_original("jean@acme.fr");
        assert_eq!(pseudo, Some("paul@example.com".to_string()));
    }
    ...
    fn test_concurrent_access() {
        ...
        // Vérifier que chaque entrée est retrouvable
        for i in 0..10 {
            let orig = format!("user{}@test.com", i);
            let pseudo = format!("fake{}@example.com", i);
